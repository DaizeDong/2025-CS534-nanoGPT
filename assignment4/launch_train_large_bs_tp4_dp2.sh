#!/usr/bin/env bash
# srun --account=bcrc-delta-gpu --qos=bcrc-delta-gpu -p gpuA100x8 -t 1:00:00 --nodes=1 --ntasks-per-node=1 --cpus-per-task=96 --gres=gpu:a100:8 --mem=256g bash assignment4/launch_train_large_bs_tp4_dp2.sh
...
# 1) LAUNCH TRAINING
#################################
torchrun --standalone --nproc_per_node=8 train.py config/train_shakespeare_char_large_bs_head8_tp4_dp2.py \
  --csv_path=runs/losses_large_bs_head8_tp4_dp2.csv  | tee -a logs/train_large_bs_head8_tp4_dp2.log
