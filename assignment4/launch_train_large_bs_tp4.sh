#!/usr/bin/env bash
# srun --account=bcrc-delta-gpu --qos=bcrc-delta-gpu -p gpuA100x4 -t 1:00:00 --nodes=1 --ntasks-per-node=1 --cpus-per-task=64 --gres=gpu:a100:4 --mem=128g bash assignment4/launch_train_large_bs_tp4.sh
...
# 1) LAUNCH TRAINING
#################################
torchrun --standalone --nproc_per_node=4 train.py config/train_shakespeare_char_large_bs_head8_tp4.py \
  --csv_path=runs/losses_large_bs_head8_tp4.csv  | tee -a logs/train_large_bs_head8_tp4.log
