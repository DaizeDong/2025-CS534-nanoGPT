#!/usr/bin/env bash
# srun --account=bcrc-delta-gpu --qos=bcrc-delta-gpu -p gpuA100x4 -t 1:00:00 --nodes=1 --ntasks-per-node=1 --cpus-per-task=64 --gres=gpu:a100:4 --mem=128g bash assignment3/launch_train_large_bs_mp4.sh
...
# 1) LAUNCH TRAINING
#################################
python train.py config/train_shakespeare_char_large_bs_mp4.py \
  --csv_path=runs/losses_large_bs_mp4.csv  | tee -a logs/train_large_bs_mp4.log
